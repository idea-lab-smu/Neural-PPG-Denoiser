...
def calc_psnr_and_ssim(sr, hr):
    ...
    img1 = np.transpose(sr.squeeze().round().cpu().numpy())
    img2 = np.transpose(hr.squeeze().round().cpu().numpy())
    img1 = img1.reshape(6*125,1,3)
    img2 = img2.reshape(6*125,1,3)
...
def calc_sqi(sr):
    ...
    mean = np.mean(img1)
    dev = np.var(img1)**0.5
    return np.sum((img1-mean/dev)**3)/(3*6*125.)