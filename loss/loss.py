...
class TPerceptualLoss(nn.Module):
    ...
    def forward(self, map_lv3, map_lv2, map_lv1, S, T_lv3, T_lv2, T_lv1):
        ...
        if (self.use_S):
            ...
            S_lv2 = torch.sigmoid(F.interpolate(S, size=(S.size(-2), S.size(-1)), mode='bicubic'))
            S_lv1 = torch.sigmoid(F.interpolate(S, size=(S.size(-2), S.size(-1)), mode='bicubic'))
        else:
...
class AdversarialLoss(nn.Module):
    def __init__(self, logger, use_cpu=False, num_gpu=1, gan_type='WGAN_GP', gan_k=1, 
        ...
        self.discriminator = discriminator.Discriminator(train_crop_size*2).to(self.device)
        if (num_gpu > 1):
