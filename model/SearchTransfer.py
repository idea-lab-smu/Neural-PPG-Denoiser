...
class SearchTransfer(nn.Module):
    ...
    def forward(self, lrsr_lv3, refsr_lv3, ref_lv1, ref_lv2, ref_lv3):
        ...
        ref_lv2_unfold = F.unfold(ref_lv2, kernel_size=(5, 5), padding=2, stride=1)
        ref_lv1_unfold = F.unfold(ref_lv1, kernel_size=(9, 9), padding=4, stride=1)
        ...
        T_lv2 = F.fold(T_lv2_unfold, output_size=(lrsr_lv3.size(2), lrsr_lv3.size(3)), kernel_size=(5,5), padding=2, stride=1) / (3.*3.)
        T_lv1 = F.fold(T_lv1_unfold, output_size=(lrsr_lv3.size(2), lrsr_lv3.size(3)), kernel_size=(9,9), padding=4, stride=1) / (3.*3.)
